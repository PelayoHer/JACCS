...
* **Mid Game (< Get Lucky)**: Dynamically calculates and hoards exactly 15 minutes of pure base CPS (6000x multiplier) to maximize normal Lucky combos. The underlying math strips (divides out) temporary Frenzy or Building Special multipliers out of the total CPS to prevent the Safe Bank from inflating to impossible levels unnecessarily.
* **Late Game (> Get Lucky)**: Hoards 105 minutes (42000x multiplier) for Frenzy+Lucky combos.
* **The Override**: If an upgrade pays for itself faster than the safe bank regenerates (e.g. `ROI < 6000s`), JACCS bypasses the hoarding lock and force-buys it. A secondary **Golden Exception** forces the instant purchase of critical zero-CPS-delta game mechanics (like *Lucky Day* and *Get Lucky*) that would normally be ignored by raw math algorithms.

### 3. Absolute Ascension Autonomy (`src/AutoAscend.js`)
The bot uses exponential Logarithmic derivatives to know precisely when to reincarnate (when Heavenly Chips yield > `10x` multiplier of current prestige). However, the engine hard-locks the very first ascension of any save file, forcing an absolute minimum of **440 Heavenly Chips** before firing, strictly obeying the Speedrunner standard to ensure enough prestige currency for legacy essentials. Upon reaching the astral screen, it:
...
* Maps to native Roman Numerals (I-V) to equip your most powerful Kitten, Fingers, Omelette, Lucky Day, and Butter Biscuit into the **5 Permanent Upgrade Slots** to instantly propel early-game speed.
...
* **Botanical Harvester & Weedkiller**: Auto-harvests mature, non-immortal plants exactly when ready to optimize seed mutation grinds. Features an intelligent pesticide sub-routine that continuously scans for invasive weeds (like *Meddleweed*), instantly eradicating them to protect your mutation crossbreeding grids (but ONLY if you've already unlocked that weed's seed).
